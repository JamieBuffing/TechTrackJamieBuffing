  ...
  try {
    ...
    await Promise.all(
      played.map(async (g) => {
        ...
        const primaryGenre = details?.genres?.[0]?.description || "Overige";
    ...
    if (!genreMap.size) {
      return json({
        steamid,
        genres: [],
        message: "Geen bruikbare genre-informatie gevonden op de store-pagina’s."
      });
    }
    const totalHours = totalMinutes / 60;
    let categoryList = [...genreMap.entries()].map(([genre, info]) => {
      const genreHours = info.hours;
      const percentage = totalHours ? Number((genreHours / totalHours * 100).toFixed(1)) : 0;
      const gamesList = info.games.sort((a, b) => b.hours - a.hours).slice(0, 15).map((g) => ({
        name: g.name,
    ...
    categoryList = categoryList.sort((a, b) => b.hours - a.hours).slice(0, 12);
