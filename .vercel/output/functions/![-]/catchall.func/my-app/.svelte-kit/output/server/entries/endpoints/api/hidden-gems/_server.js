...
import { r as resolveSteamId, d as getOwnedGames, f as getReviewSummary } from "../../../../chunks/steamApi.js";
async function GET({ url, fetch }) {
  try {
    ...
    const lowPlayed = games.filter((g) => {
    ...
    await Promise.all(
      lowPlayed.map(async (game) => {
        ...
        const positiveRatio = totalReviews > 0 ? Math.round(totalPositive / totalReviews * 100) : null;
        const minutes = game.playtime_forever || 0;
        const hours = minutes / 60;
        if (!totalReviews || totalReviews === 0) return;
        const ratingNorm = reviewScore != null ? Math.min(Math.max(reviewScore / 9, 0), 1) : 0;
        const popularityNorm = Math.min(totalReviews / 5e3, 1);
        let lowPlayNorm = 0;
        if (hours <= 0.5) {
          lowPlayNorm = 1;
        } else if (hours < 5) {
        ...
        const baseScore = 0.5 * ratingNorm + 0.3 * popularityNorm + 0.2 * lowPlayNorm;
    ...
    if (!gems.length) {
      return json({
        steamid,
        gems: [],
        message: "Steam gaf geen voldoende rating/review-data om hidden gems te bepalen."
      });
    }
